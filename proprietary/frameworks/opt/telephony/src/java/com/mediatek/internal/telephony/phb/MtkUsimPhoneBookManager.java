...
public class MtkUsimPhoneBookManager extends UsimPhoneBookManager {
    ...
    private void readAdnFileAndWait(int recId) {
        logi("readAdnFileAndWait begin: recId is " + recId + "");
        ...
        logi("readAdnFileAndWait end: recId is " + recId + "");
    }
    ...
    public boolean addContactToGroup(int adnIndex, int grpIndex) {
        boolean ret = false;
        logi("addContactToGroup begin adnIndex is " +
    ...
    public synchronized boolean removeContactFromGroup(int adnIndex, int grpIndex) {
        boolean ret = false;
        logi("removeContactFromGroup begin adnIndex is " +
                adnIndex + " to grp " + grpIndex);
        if (mPhoneBookRecords == null || adnIndex <= 0 || adnIndex > mPhoneBookRecords.size()) {
        ...
        logi("removeContactFromGroup end adnIndex is " +
                adnIndex + " to grp " + grpIndex);
        return ret;
    }
    ...
    private void readType1Ef(File file, int anrIndex) {
        ...
        logi("readType1Ef before mLock.wait " + mNeedNotify.get()
            + " total:" + totalReadingNum);
        synchronized (mLock) {
            try {
                mLock.wait();
    ...
    private void readType2Ef(File file) {
        ...
        logi("readType2Ef before mLock.wait " + mNeedNotify.get()
            + " total:" + totalReadingNum);
        synchronized (mLock) {
            try {
                mLock.wait();
    ...
    public int updateSneByAdnIndex(String sne, int adnIndex, MtkAdnRecord oldAdn) {
        ...
        logi("updateSneByAdnIndex begin sne is " + sne + ",adnIndex "
                + adnIndex);
        int pbrRecNum = (adnIndex - 1) / mAdnFileSize;
