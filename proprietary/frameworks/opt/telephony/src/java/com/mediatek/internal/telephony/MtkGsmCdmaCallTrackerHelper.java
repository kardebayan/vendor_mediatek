...
public final class MtkGsmCdmaCallTrackerHelper
    ...
    /// M: CC: MTK incoming call number check for reject call at EAIC phase.
    ...
    public void CallIndicationProcess(AsyncResult ar, boolean isIncomingNumberChecked,
        ...
        if (mMtkTracker.mState == PhoneConstants.State.RINGING) {
            mode = 1;
            rejectCause = MtkDisconnectCause.INCOMING_MISSED;
        }
        ...
        if (mode == 0) {
            if (isIncomingNumberChecked) {
                ...
                if (SubscriptionManager.isValidSubscriptionId(subId) &&
                    ...
                    mIncomingCallCheker = new MtkIncomingCallChecker(""+callId+"_"+seqNumber, ar);
                    boolean bCheckStart = mIncomingCallCheker.startIncomingCallNumberCheck(mContext,
                            subId, number, this);
                    if (bCheckStart) {
                        ...
                        logE("startIncomingCallNumberCheck false, and flow continues. (callId_seqNo="
                                + callId + "_" + seqNumber + ", subId=" + subId + ", number="
                                + number + ")");
                    }
                }
        ...
        if (mode == 0) {
            ...
            mMtkTracker.mMtkCi.setCallIndication(mode, callId, seqNumber, null);
        }
        ...
        if (mode == 1) {
            ...
            if (rejectCause != MtkDisconnectCause.INCOMING_MISSED) {
                mMtkTracker.mMtkCi.setCallIndicationWithCause(mode, callId, seqNumber, null,
                        rejectCause);

                if (rejectCause == MtkDisconnectCause.INCOMING_REJECTED) {
                    addCallLog(mContext, mMtkTracker.mPhone.getIccSerialNumber(), number, false,
            ...
            dc.number = MtkPhoneNumberUtils.stringFromStringAndTOA(dc.number, dc.TOA);

            MtkGsmCdmaConnection cn = new MtkGsmCdmaConnection(mMtkTracker.mPhone, dc, mMtkTracker,
                    callId);
            cn.onReplaceDisconnect(rejectCause);
        }
        /// @}
    }
    ...
    private void addCallLog(Context context, String iccId, String number, boolean isVideo) {
        addCallLog(context, iccId, number, isVideo, CallLog.Calls.MISSED_TYPE);
    }
    ...
    private void addCallLog(Context context, String iccId, String number, boolean isVideo,
        ...
        CallLog.Calls.addCall(null, context, number,
                presentationMode, callType, features,
                phoneAccountHandle,
