...
public class MtkImsSmsDispatcher extends ImsSmsDispatcher {
    ...
    protected void sendData(String destAddr, String scAddr, int destPort, int originalPort,
        ...
        if (!isCdmaMo()) {
            sendDataGsm(destAddr, scAddr, destPort, originalPort, data, sentIntent,
                    deliveryIntent);
        } else {
    ...
    public void sendMultipartData(
        ...
        if (!isCdmaMo()) {
            sendMultipartDataGsm(destAddr, scAddr, destPort, data, sentIntents,
                    deliveryIntents);
        } else {
    ...
    protected SmsTracker getNewSubmitPduTracker(String destinationAddress, String scAddress,
        ...
        if (isCdmaMo()) {
            return super.getNewSubmitPduTracker(destinationAddress, scAddress, message, smsHeader,
                    encoding, sentIntent, deliveryIntent, lastPart, unsentPartCount, anyPartFailed,
                    messageUri, fullMessageText, priority, expectMore, validityPeriod);
        } else {
            return getNewSubmitPduTrackerGsm(destinationAddress, scAddress, message, smsHeader,
                    encoding, sentIntent, deliveryIntent, lastPart, unsentPartCount, anyPartFailed,
                    messageUri, fullMessageText, priority, expectMore, validityPeriod);
        }
    }
   ...
   // GSM-START
     ...
     *  raw pdu of the status report is in the extended data ("pdu").
     */
    protected void sendDataGsm(String destAddr, String scAddr, int destPort, int originalPort,
            byte[] data, PendingIntent sentIntent, PendingIntent deliveryIntent) {
    ...
    /** {@inheritDoc} */
    public void sendMultipartDataGsm(
            String destAddr, String scAddr, int destPort,
            ArrayList<SmsRawData> data, ArrayList<PendingIntent> sentIntents,
            ArrayList<PendingIntent> deliveryIntents) {
        if (data == null) {
    ...
    protected SmsTracker getSmsTracker(HashMap<String, Object> data, PendingIntent sentIntent,
        ...
        SmsTracker tracker = super.getSmsTracker(data, sentIntent, deliveryIntent, format,
    ...
    private SmsTracker getNewSubmitPduTrackerGsm(String destinationAddress, String scAddress,
            String message, SmsHeader smsHeader, int encoding,
            PendingIntent sentIntent, PendingIntent deliveryIntent, boolean lastPart,
            AtomicInteger unsentPartCount, AtomicBoolean anyPartFailed, Uri messageUri,
            String fullMessageText, int priority, boolean expectMore, int validityPeriod) {
