...
public final class MtkGsmMmiCode extends GsmMmiCode {
    ...
    public static MtkGsmMmiCode newFromDialString(String dialString, MtkGsmCdmaPhone phone,
        ...
        Rlog.d(LOG_TAG, "newFromDialString, dialstring = " + dialString);
...
*/
    ...
    public void processCode() throws CallStateException {
        try {
            if ((supportMdAutoSetupIms() == false) &&
                    (mPhone.isDuringVoLteCall() || mPhone.isDuringImsEccCall())) {
    ...
    void handleCallForward() {
        ...
        if (isInterrogate()) {
            ...
            if (includeCFU ==1) {
                mOrigUtCfuMode = 0;
                String utCfuMode = TelephonyManager.getTelephonyProperty(mPhone.getPhoneId(),
    ...
    public static MtkGsmMmiCode newNetworkInitiatedUssd(String ussdMessage,
            boolean isUssdRequest, MtkGsmCdmaPhone phone, UiccCardApplication app) {
        MtkGsmMmiCode ret;
    ...
    protected CharSequence getErrorMessage(AsyncResult ar) {
        if (ar.exception instanceof CommandException) {
            CommandException.Error err = ((CommandException)(ar.exception)).getCommandError();
            /* MTK has reserved OEM_ERROR_1 for HTTP 409 conflict error, so change AOSP error code
             * from Error.OEM_ERROR_1 to Error.OEM_ERROR_10 */
            ...
            } else if (err == CommandException.Error.OEM_ERROR_10) {
