...
public class WorldPhoneOp01 extends Handler implements IWorldPhone {
    ...
    private void handleServiceStateChange(AsyncResult ar, int slotId) {
        ...
        if (ar.exception == null && ar.result != null) {
            ...
            if (sServiceState != null) {
                ...
                logd("slotId: " + slotId + ", " +
                        WorldPhoneUtil.iccCardTypeToString(sIccCardType[slotId]) + ", " +
                        "sMajorSim: " + sMajorSim + ", " +
                        "sPlmnSs: " + sPlmnSs + ", " +
                        "sVoiceRegState: " + WorldPhoneUtil.stateToString(sVoiceRegState));
                logd("sRilVoiceRegState: " + sRilVoiceRegState + ", " +
                        "sRilVoiceRadioTech: " +
                        sServiceState.rilRadioTechnologyToString(sRilVoiceRadioTechnology) + ", " +
                        "sDataRegState: " + WorldPhoneUtil.stateToString(sDataRegState));
    ...
    private boolean isNoService() {
        ...
        if (sVoiceRegState == MtkServiceState.STATE_OUT_OF_SERVICE
                && (sRilVoiceRegState == NetworkRegistrationState.REG_STATE_NOT_REG_NOT_SEARCHING
               || sRilVoiceRegState == MtkServiceState.
                ...
                && sRilDataRegState == NetworkRegistrationState.REG_STATE_NOT_REG_NOT_SEARCHING) {
            noService = true;
        } else {
            noService = false;
        }
    ...
    private int getIccCardType(int slotId) {
        ...
        simString = ((sProxyPhones[slotId].getIccCard())).getIccCardType();
    ...
    private void resumeCampingProcedure(int slotId, boolean isResumeModem) {
        ...
        if (isAllowCampOn(plmnString, slotId) || (isNeedSwitchModem() == false)) {
            removeModemStandByTimer();
