...
// MTK-START: SS
import static
        com.mediatek.internal.telephony.MtkTelephonyProperties.PROPERTY_UT_CFU_NOTIFICATION_MODE;
...
import static
        com.mediatek.internal.telephony.MtkTelephonyProperties.UT_CFU_NOTIFICATION_MODE_DISABLED;
// MTK-END
...
public class MtkSIMRecords extends SIMRecords {
    ...
    */
    ...
    public void dispose() {
        ...
        if (ssHelper != null) {
            ssHelper.sendMessage(ssHelper.obtainMessage(MtkSuppServHelper.EVENT_CLEAN_CFU_STATUS));
        }
        // MTK-END
        super.dispose();
    }
    ...
    public boolean checkEfCff() {
        boolean isValid = mEfCff != null;
        mtkLog("mEfCff is null? = " + (mEfCff == null));
        return isValid;
    }
    // MTK-END
    ...
    public String getVoiceMailNumber() {
        mtkLog("getVoiceMailNumber " + mVoiceMailNum);
    ...
    public void setVoiceCallForwardingFlag(int line, boolean enable, String dialNumber) {
        ...
        setCFUStatusToLocal(enable);  // Write to system property.
        ...
        try { switch (msg.what) {
            ...
            case EVENT_GET_IMSI_DONE:
                ...
                if (ar.exception != null) {
                    mtkLoge("Exception querying IMSI, Exception:" + ar.exception);
                    break;
                }
                ...
                if (mImsi != null && (mImsi.length() < 6 || mImsi.length() > 15)) {
                    mtkLoge("invalid IMSI " + mImsi);
                    mImsi = null;
                }

                mtkLog("IMSI: mMncLength=" + mMncLength);
                if (mImsi != null) {
                    mtkLog("IMSI: " + mImsi.substring(0, 6) +
                ...
                // MTK-END
                    ...
                    for (String mccmnc : MCCMNC_CODES_HAVING_3DIGITS_MNC) {
                        if (mccmnc.equals(mccmncCode)) {
                            mMncLength = 3;
                            mtkLog("IMSI: setting1 mMncLength=" + mMncLength);
                            break;
                        }
                    }
                }
                ...
                // MTK-END
                    ...
                    try {
                        ...
                        mtkLog("setting2 mMncLength=" + mMncLength);
                    } catch (NumberFormatException e) {
                        mMncLength = UNKNOWN;
                        mtkLoge("Corrupt IMSI! setting3 mMncLength=" + mMncLength);
                    }
                }

                if (mMncLength != UNKNOWN && mMncLength != UNINITIALIZED) {
                    mtkLog("update mccmnc=" + imsi.substring(0, 3 + mMncLength));
            ...
            case EVENT_GET_MBI_DONE:
                ...
                case EVENT_GET_AD_DONE:
                    try {
                        ...
                        mtkLog("EF_AD: " + IccUtils.bytesToHexString(data));
                        ...
                        mtkLog("setting4 mMncLength=" + mMncLength);

                        if (mMncLength == 0xf) {
                            mMncLength = UNKNOWN;
                            mtkLog("setting5 mMncLength=" + mMncLength);
                        ...
                        if (((mMncLength == UNINITIALIZED) || (mMncLength == UNKNOWN) ||
                                (mMncLength == 2)) && ((imsi != null) && (imsi.length() >= 6))) {
                            String mccmncCode = imsi.substring(0, 6);
                            mtkLog("mccmncCode=" + mccmncCode);
                            for (String mccmnc : MCCMNC_CODES_HAVING_3DIGITS_MNC) {
                                if (mccmnc.equals(mccmncCode)) {
                                    mMncLength = 3;
                                    mtkLog("setting6 mMncLength=" + mMncLength);
                                    break;
                                }
                            }
                        }
                        ...
                        if (mMncLength == UNKNOWN || mMncLength == UNINITIALIZED) {
                            if (imsi != null) {
                                try {
                                    ...
                                    mMncLength = MccTable.smallestDigitsMccForMnc(mcc);
                                    mtkLog("setting7 mMncLength=" + mMncLength);
                                } catch (NumberFormatException e) {
                                    mMncLength = UNKNOWN;
                                    mtkLoge("Corrupt IMSI! setting8 mMncLength=" + mMncLength);
                                }
                            } else {
                                // Indicate we got this info, but it didn't contain the length.
                                mMncLength = UNKNOWN;
                                mtkLog("MNC length not present in EF_AD setting9 mMncLength=" +
                                        mMncLength);
                            }
                        }
                        ...
                        // MTK-START
                        // To get mcc mnc as early as possible if ESIMAPP does not support by MD.
                        String operator = getOperatorNumeric();
                        if (!TextUtils.isEmpty(operator)) {
                            mtkLog("EVENT_GET_AD_DONE set mccmnc to operator='" +
                                    operator + "'");
                            SystemProperties.set(SIM_RECORDS_PROPERTY_MCC_MNC[mSlotId], operator);
                        } else {
                ...
                case EVENT_GET_SPN_DONE:
                    // MTK-START
                    if (DBG) mtkLog("EF_SPN loaded and try to extract: ");
                    // MTK-END
                    isRecordLoadResponse = true;
                    ar = (AsyncResult) msg.obj;
                    // MTK-START
                    ...
                    if (ar != null && ar.exception == null) {
                        if (DBG) mtkLog("getSpnFsm, Got data from EF_SPN");
                        data = (byte[]) ar.result;
                        ...
                        if (mSpNameInEfSpn != null && mSpNameInEfSpn.equals("")) {// MVNO-API
                            if (DBG) {
                ...
                case EVENT_GET_ALL_PNN_DONE:
                    ...
                    if (subInfo != null && subInfo.getNameSource() !=
                        ...
                        String simMvnoName = MtkSpnOverride.getInstance().
                                lookupOperatorNameForDisplayName(
                                subId, operator, true, mContext);
                        if (!TextUtils.isEmpty(simMvnoName)) {
                            SubscriptionController.getInstance().setDisplayNameUsingSrc(
                                    simMvnoName, subId,
                                    SubscriptionManager.NAME_SOURCE_UNDEFINDED);
                        }
                    }
                    //// M: }
                   if (!mReadingOpl) {
                ...
                */
                ...
                case EVENT_IMSI_REFRESH_QUERY_DONE:
                    ...
                    //reset the mnc length and update it by new imsi.
                    mMncLength = UNKNOWN;
                    ...
                    if (((mMncLength == UNINITIALIZED) ||
                            (mMncLength == UNKNOWN) || (mMncLength == 2)) &&
                            ((imsi != null) && (imsi.length() >= 6))) {
                        String mccmncRefresh = imsi.substring(0, 6);
                    ...
                    if (mMncLength == UNKNOWN || mMncLength == UNINITIALIZED) {
                        ...
                        // guess using the mcc
                        try {
                            int mccR = Integer.parseInt(imsi.substring(0, 3));
                    ...
                    if (mMncLength != UNKNOWN && mMncLength != UNINITIALIZED) {
                        mtkLog("update mccmnc=" + imsi.substring(0, 3 + mMncLength));
                        //finally have both the imsi and the mncLength and can parse the
                        //imsi properly.
                        //MccTable.updateMccMncConfiguration(mContext,
                                //imsi.substring(0, 3 + mMncLength), false);
                        updateConfiguration(imsi.substring(0, 3 + mMncLength));
                    }
                    if (!imsi.equals(mSimImsi)) {
                        mSimImsi = imsi;
                        mImsiReadyRegistrants.notifyRegistrants();
                ...
                case EVENT_PHB_READY:
                    ...
                    if (ar != null && ar.exception == null && ar.result != null) {
                        ...
                        if (DBG)
                            mtkLog("isPhbReady=" + isPhbReady[0] + ",curSimState = " +
    ...
    protected void onAllRecordsLoaded() {
        ...
        if (DBG) {
            if (USERDEBUG) {
                mtkLog("imsi = *** operator = " + operator);
            } else {
                mtkLog("imsi = " + getIMSI() + " operator = " + operator);
            }
        }
    ...
    private class SIMBroadCastReceiver extends BroadcastReceiver {
        public void onReceive(Context content, Intent intent) {
            ...
            if (action.equals(TelephonyIntents.ACTION_SIM_STATE_CHANGED)) {
                ...
                mtkLog("SIM_STATE_CHANGED: slot = " + slot + ",reason = " + reasonExtra +
                        ",simState = " + simState);
    ...
    */
    ...
    parseEfSpdi(byte[] data) {
        ...
        for (int i = 0 ; i + 2 < plmnEntries.length ; i += 3) {
            ...
            plmnCode = MtkIccUtils.parsePlmnToString(plmnEntries, i, 3);
            // MTK-END

            // Valid operator codes are 5 or 6 digits
            if (plmnCode.length() >= 5) {
                mtkLog("EF_SPDI network: " + plmnCode);
    ...
    public void broadcastPhbStateChangedIntent(boolean isReady) {
        ...
        mtkLog("broadcastPhbStateChangedIntent, mPhbReady " + mPhbReady);
        if (isReady == true) {
    ...
    public boolean isPhbReady() {
        if (DBG) mtkLog("isPhbReady(): cached mPhbReady = " + (mPhbReady ? "true" : "false"));
        ...
        if (DBG) mtkLog("isPhbReady(): mPhbReady = " + (mPhbReady ? "true" : "false") +
            ...
            if ((mPhbWaitSub == true) &&
                ...
                if (null != mParentApp && mParentApp.getPhoneId() == phoneid) {
                    ...
                    if (!cdmaPhoneName.equals(activePhoneName)) {
                        broadcastPhbStateChangedIntent(false);
    ...
    protected void updatePHBStatus(int status, boolean isSimLocked) {
        mtkLog("[updatePHBStatus] status : " + status + " | isSimLocked : " + isSimLocked
                + " | mPhbReady : " + mPhbReady);
    ...
    protected void setVoiceCallForwardingFlagFromSimRecords() {
        if (checkEfCfis()) {
            ...
            log("EF_CFIS2: callForwardingEnabled=" + mCallForwardingStatus);
            mRecordsEventsRegistrants.notifyResult(EVENT_CFI);
        } else if (mEfCff != null) {
            ...
            log("EF_CFF2: callForwardingEnabled=" + mCallForwardingStatus);
            mRecordsEventsRegistrants.notifyResult(EVENT_CFI);
        } else {
    ...
    private void setCFUStatusToLocal(boolean status) {
        log("setCFUStatusToLocal: " + status);
        if (status) {
