...
public class MtkImsPhone extends ImsPhone {
    ...
    public boolean isNotSupportUSSIOverCall() {
        // Mobitel.
        final List<String> OP232_MCCMNC_LIST = Arrays.asList("41301");
        ...
        logd(" current mccMnc = " + mccMnc);
        boolean ret = OP232_MCCMNC_LIST.contains(mccMnc);
        return ret;
    }
    ...
    protected Connection dialInternal(String dialString, DialArgs dialArgs,
        ...
        if (!PhoneNumberUtils.isUriNumber(dialString)) {
            mmi = MtkImsPhoneMmiCode.newFromDialString(networkPortion, this);
        } else {
    ...
    protected void onIncomingUSSD(int ussdMode, String ussdMessage) {
        ...
        isUssdErrorWithNoCSFB = (ussdMode == -2);
    ...
    public void onMMIDone(ImsPhoneMmiCode mmi) {
        /**
         * Get GSM's service state from GsmCdmaPhone's service state tracker
         * If the state is not in service, do not retry USSD over CS.
         * We should also do null check for SST, because SST may not be created yet.
         */
        int gsmServiceState = ServiceState.STATE_OUT_OF_SERVICE;
        if (mDefaultPhone.getServiceStateTracker() != null) {
            gsmServiceState = mDefaultPhone.getServiceStateTracker().mSS.getState();
        }

        /// M: @{
        logd("onMMIDone: " + mmi + ", mUssiCSFB=" + mUssiCSFB +
                ", isInCall=" + (isInCall() ? "true" : "false") +
                ", gsmServiceState=" + gsmServiceState);
        dumpPendingMmi();
        /// @}
        /// M: Set USSI as CSFB @{
        if (mUssiCSFB && !(isInCall() && isVolteEnabled()) &&
                (gsmServiceState == ServiceState.STATE_IN_SERVICE)) {
            String dialString = ((MtkImsPhoneMmiCode)mmi).getUssdDialString();
            if (dialString != null && !dialString.isEmpty()) {
        ...
        // This time of USSI seesion is done, reset mUssiCSFB to default value
        mUssiCSFB = false;
        super.onMMIDone(mmi);
    }
    ...
    protected void updateRoamingState(boolean newRoaming) {
        if (mCT.getState() == PhoneConstants.State.IDLE) {
            if (DBG) logd("updateRoamingState now: " + newRoaming +
                ", isWfcEnabled:" + isWifiCallingEnabled());

            // IWLAN is treated as home (roaming == false) and it will cause
            // AP sets home wfc mode (cellular prefer) to modem when WFC is registered
            // in vzw romaing environment. So we ignore the roaming state change here
            if (isWifiCallingEnabled()) {
                if (DBG) logd("ignore due to IWLAN is treated as home");
                return;
            }
            ...
            if (mIsWfcModeHomeForDomRoaming) {
                ServiceState ss = getDefaultPhone().getServiceState();
                int voiceRoamingType = ss.getVoiceRoamingType();
                int dataRoamingType = ss.getDataRoamingType();

                if (newRoaming && (voiceRoamingType == ServiceState.ROAMING_TYPE_DOMESTIC ||
                    ...
                    MtkImsManager.setWfcMode(mContext,
                            MtkImsManager.getWfcMode(mContext, false, mPhoneId), false, mPhoneId);
                    return;
                }
            }

            MtkImsManager.setWfcMode(mContext,
                    MtkImsManager.getWfcMode(mContext, newRoaming, mPhoneId), newRoaming, mPhoneId);
        } else {
    ...
    public void sendUssdResponse(String ussdMessge) {
        ...
        mPendingMMIs.add(mmi);
        /// M: @{
        logd("sendUssdResponse: " + ussdMessge + ", mmi=" + mmi);
        dumpPendingMmi();
        /// @}
        mMmiRegistrants.notifyRegistrants(new AsyncResult(null, mmi, null));
    ...
    private void cacheCarrierConfiguration() {
        ...
        logd("cacheCarrierConfiguration, "
            ...
            + " mIsWfcModeHomeForDomRoaming: " + mIsWfcModeHomeForDomRoaming);
    }
        ...
        if (feature == FeatureType.VOLTE_ENHANCED_CONFERENCE
            ...
            if (feature == FeatureType.VOLTE_ECT) {
                if (mIsBlindAssuredEctSupported) {
                    logd("vodafone isFeatureSupported(" + feature + "): true"
