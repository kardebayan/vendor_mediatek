...
public class OpTelephonyCustomizationUtils {
    ...
    // Operator common telephony factory - START
    ...
    public static OpCommonTelephonyCustFactoryBase getOpCommonInstance() {
        if (sInstance == null) {
            synchronized (OpCommonTelephonyCustFactoryBase.class) {
                // the corresponding OP common factory class
                String className = "com.mediatek.op.telephony.OpCommonTelephonyCustFactory";

                Class<?> clazz = null;
                try {
                    // new OP common factory by java reflection
                    // if the jar is already in PRODUCT_BOOT_JARS,
                    // just call Class.forName(className)
                    clazz = Class.forName(className);
                    Constructor clazzConstructFunc = clazz.getConstructor();
                    sInstance = (OpCommonTelephonyCustFactoryBase) clazzConstructFunc.newInstance();
                    Rlog.d(DBG_TAG, "Success to get OpCommonTelephonyCustFactoryBase class");
                } catch (Exception e) {
                    // if no OP common jar, back to default behavior
    ...
    // Operator common telephony factory - END
}
