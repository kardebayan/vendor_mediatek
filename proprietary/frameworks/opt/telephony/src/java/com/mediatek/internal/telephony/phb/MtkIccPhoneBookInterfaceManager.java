...
public class MtkIccPhoneBookInterfaceManager extends IccPhoneBookInterfaceManager {
                ...
                case EVENT_LOAD_DONE:
                    ...
                    synchronized (mLock) {
                        if (ar.exception == null) {
                            mRecords = (List<MtkAdnRecord>) ar.result;
                        } else {
    ...
    public synchronized int updateAdnRecordsInEfBySearchWithError(int efid,
        ...
        synchronized (mLock) {
            ...
            Message response = mMtkBaseHandler.obtainMessage(EVENT_UPDATE_DONE, status);
    ...
    public synchronized int updateUsimPBRecordsInEfBySearchWithError(int efid,
        ...
        if (DBG) {
            logd("updateUsimPBRecordsInEfBySearchWithError: efid=" + efid +
                    ...
                    + " newGrpIds = " + newGrpIds
                    + " newEmails = " + ((newEmails == null) ? "null" : newEmails[0]));
        }
        synchronized (mLock) {
            checkThread();
            mSuccess = false;
    ...
    public int[] getAdnRecordsSize(int efid) {
        ...
        synchronized (mLock) {
            ...
            AtomicBoolean status = new AtomicBoolean(false);
            Message response = mMtkBaseHandler.obtainMessage(EVENT_GET_SIZE_DONE, status);
    ...
    public synchronized List<MtkAdnRecord> getAdnRecordsInEf(int efid, Object object) {
        ...
        synchronized (mLock) {
            checkThread();
            AtomicBoolean status = new AtomicBoolean(false);
            Message response = mMtkBaseHandler.obtainMessage(EVENT_LOAD_DONE, status);
            if (mAdnCache != null) {
