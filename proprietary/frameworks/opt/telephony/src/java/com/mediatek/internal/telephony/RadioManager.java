...
public class RadioManager extends Handler  {
    ...
    protected RadioManager(Context context , int phoneCount, CommandsInterface[] ci) {
        ...
        ((MtkRIL)mCi[mainPhoneId]).reportAirplaneMode(airplaneMode,
    ...
    private void setRadioPowerAfterCapabilitySwitch() {
        ...
        // turn on radio
        // turn on main protocol first
        int mainPhoneId = RadioCapabilitySwitchUtil.getMainCapabilityPhoneId();
        setRadioPower(!mAirplaneMode, mainPhoneId);
   ...
   /**
    ...
    public void notifyAirplaneModeChange(boolean enabled) {
        ...
        ((MtkRIL)mCi[phoneId]).reportAirplaneMode(enabled ? 1 : 0,
                obtainMessage(EVENT_REPORT_AIRPLANE_DONE));
        ...
        if (radioAction == TO_SET_RADIO_POWER) {
            ...
            // turn on main protocol first
            int mainPhoneId = RadioCapabilitySwitchUtil.getMainCapabilityPhoneId();
            setRadioPower(radioPower, mainPhoneId);
            for (int i = 0; i < mPhoneCount; i++) {
                if (mainPhoneId != i) {
                    setRadioPower(radioPower, i);
                }
    ...
    public void setSilentRebootPropertyForAllModem(String isSilentReboot) {
        ...
        ((MtkRIL)mCi[phoneId]).setSilentReboot(on,
    ...
    protected void refreshSimSetting(boolean radioPower, int phoneId) {
        ...
        ((MtkRIL)mCi[findMainCapabilityPhoneId()]).reportSimMode(mSimModeSetting,
        ...
        if ((ar.exception == null) && (ar.result != null)) {
            ...
            if (findMainCapabilityPhoneId() == phoneId) {
                if (state == DsbpState.DSBP_ENHANCEMENT_START) {
                    mIsMainProDsbpChanging = true;
                } else {
                    mIsMainProDsbpChanging = false;
    ...
    protected boolean isModemOff(int phoneId) {
        ...
        switch(config) {
            case DSDS:
                powerOff = !SystemProperties.get("vendor.ril.ipo.radiooff").equals("0");
                break;
            case DSDA:
                switch (phoneId) {
                    case 0: //phone 1
                        ...
                        break;
                    case 1: //phone 2
                        powerOff = !SystemProperties.get("vendor.ril.ipo.radiooff.2").equals("0");
                        break;
                    default:
                        powerOff = true;
                        break;
