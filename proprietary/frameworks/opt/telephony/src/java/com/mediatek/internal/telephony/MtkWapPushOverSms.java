...
public class MtkWapPushOverSms extends WapPushOverSms {
    ...
    private DecodedResult decodeWapPdu(byte[] pdu, InboundSmsHandler handler) {
        ...
        try {
            ...
            MtkWspTypeDecoder pduDecoder =
                    (MtkWspTypeDecoder)(TelephonyComponentFactory.getInstance().makeWspTypeDecoder(
                            pdu));
    ...
    public int dispatchWapPdu(byte[] pdu,  BroadcastReceiver receiver, InboundSmsHandler handler,
        ...
        if (ENG) Rlog.i(TAG, "dispathchWapPdu!"
            + extra.getString(MtkTelephony.WapPush.ADDR) + " "
            + extra.getString(MtkTelephony.WapPush.SERVICE_ADDR));
