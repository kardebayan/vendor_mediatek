...
public class MtkGsmCdmaCallTracker extends GsmCdmaCallTracker {
    ...
    private final BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(TelephonyManager.ACTION_SIM_CARD_STATE_CHANGED)) {
                ...
                if ((TelephonyManager.SIM_STATE_PRESENT == simState) &&
    ...
    protected void reset() {
        ...
        if (VDBG) Rlog.d(LOG_TAG, "For 93 or Vzw");
    ...
    protected synchronized void handlePollCalls(AsyncResult ar) {
        ...
        boolean needNotifyCdmaCallAccepted = false;
        ...
        for (int i = 0, curDC = 0, dcSize = polledCalls.size()
            ...
            if (conn == null && dc != null) {
                ...
                if (mPendingMO != null && mPendingMO.compareTo(dc)) {
                    ...
                    /// M: CC: CDMA call accepted @{
                    if (!isPhoneTypeGsm()
                            && mConnections[i] instanceof MtkGsmCdmaConnection
                            && ((MtkGsmCdmaConnection) mConnections[i]).isRealConnected()) {
                        needNotifyCdmaCallAccepted = true;
                    }
                    /// @}
                ...
                if (!isPhoneTypeGsm() && conn.isIncoming() != dc.isMT) {
                    ...
                    /// M: CC: CDMA call accepted
                    boolean realConnectedBeforeChange = conn instanceof MtkGsmCdmaConnection
                            && ((MtkGsmCdmaConnection) conn).isRealConnected();
                    ...
                    /// M: CC: CDMA call accepted @{
                    if (!isPhoneTypeGsm() && !realConnectedBeforeChange
                            && conn instanceof MtkGsmCdmaConnection
                            && ((MtkGsmCdmaConnection) conn).isRealConnected()) {
                        needNotifyCdmaCallAccepted = true;
                    }
                    /// @}
        ...
        /// M: CC: CDMA call accepted @{
        if (!isPhoneTypeGsm() && mPhone instanceof MtkGsmCdmaPhone && needNotifyCdmaCallAccepted) {
    ...
    public void handleMessage(Message msg) {
        ...
        case EVENT_RADIO_OFF_OR_NOT_AVAILABLE:
            ...
            if (mPhone.isInEcm() && mPhone instanceof MtkGsmCdmaPhone) {
                ((MtkGsmCdmaPhone) mPhone).sendExitEmergencyCallbackModeMessage();
            }
        break;
        /// @}
    ...
    protected boolean canDial() {
        if (!isPhoneTypeGsm() && mForegroundCall.getState() == GsmCdmaCall.State.DIALING) {
            Rlog.i(PROP_LOG_TAG, "canDial(), state: " + mForegroundCall.getState());
            return false;
        }
    ...
    private boolean shouldNotifyWaitingCall(CdmaCallWaitingNotification cw) {
        String address = cw.number;
        proprietaryLog("shouldNotifyWaitingCall");
        if (address != null && address.length() > 0) {
