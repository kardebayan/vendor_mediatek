...
public class CapabilitySwitch {
    ...
    static public void setNewSimSlot(Intent intent) {
        if (intent == null) {
            log("setNewSimSlot, intent is null => return");
            return;
        }
        int newSimSlot = intent.getIntExtra(MtkSubscriptionManager.INTENT_KEY_NEW_SIM_SLOT, 0);
        int oldSimSlot = getNewSimSlot();
        log("setNewSimSlot, new slot=" + newSimSlot + ", oldSimSlot=" + oldSimSlot);
        if (newSimSlot != oldSimSlot) {
            SystemProperties.set(NEW_SIM_SLOT, Integer.toString(newSimSlot | oldSimSlot));
        }
    }

    static public void resetNewSimSlot() {
        log("resetNewSimSlot");
        SystemProperties.set(NEW_SIM_SLOT, "");
    }

    static public int getNewSimSlot() {
        log("getNewSimSlot");
