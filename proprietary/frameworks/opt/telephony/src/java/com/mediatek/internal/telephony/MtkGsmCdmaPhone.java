...
public class MtkGsmCdmaPhone extends GsmCdmaPhone {
    ...
    public static final String REASON_WIFI_STATE_CHANGE = "wifiStateChange";
    ...
    /// M: CC: Proprietary CRSS handling
    protected static final int EVENT_CRSS_IND                       = EVENT_MTK_BASE + 2;
    /// M: CC: GSA HD Voice for 2/3G network support
    protected static final int EVENT_SPEECH_CODEC_INFO              = EVENT_MTK_BASE + 3;
     ...
     * UT_UNKNOWN_HOST                   = CommandException.Error.NO_NETWORK_FOUND
     * UT_XCAP_409_CONFLICT              = CommandException.Error.OEM_ERROR_1
    ...
    /* If receive a Network-initiated USSD Request, set this flag
     * to inidicate that there is a further action from user.
     */
    ...
    public MtkGsmCdmaPhone(Context context, CommandsInterface ci, PhoneNotifier notifier,
        ...
        mFdManager = FdManager.getInstance(this);
        ...
        /// M: CC: GSA HD Voice for 2/3G network support
        mMtkCi.setOnSpeechCodecInfo(this, EVENT_SPEECH_CODEC_INFO, null);
    ...
    /**
     * Set Network Selection by MTK
     * @param mode 1 is for none-blocking Auto, 2 is for SemiAutomatic
     * @param network is selection info
     * @param response is callback
     */
    public void setMtkNetworkSelection(int mode, OperatorInfo network, Message response) {
        if (mode == 1) {
            mMtkCi.setNetworkSelectionModeManualWithAct("00000", "0", 2, response);
        } else if (mode == 2) {
            setNetworkSelectionModeSemiAutomatic(network, response);
        } else {
            return;
        }
    }
    ...
    public void getAvailableNetworks(Message response) {
        if (isPhoneTypeGsm() || isPhoneTypeCdmaLte()) {
            mMtkCi.getAvailableNetworksWithAct(response);
        } else {
    ...
    public Connection dial(String dialString, @NonNull DialArgs dialArgs)
        ...
        /// M: CC: When selecting NoSIM CDMA phone, some numbers (000,08,118) are taken as GSM
        // phone number only, but Modem can switch phone in 93 project, so use API without subId.
        // [ALPS03567038] @{
        if (hasC2kOverImsModem() &&
                (!TelephonyManager.getDefault().hasIccCard(getPhoneId())
        ...
        /// M: is local emergency number.
        boolean isLocalEccNum = PhoneNumberUtils.isLocalEmergencyNumber(getContext(), dialString);
        boolean isEcc = (isEmergency || isLocalEccNum);
        ...
        if (DBG) {
            logd("PhoneId = " + mPhoneId
                    ...
                    + ", isLocalEmergency=" + isLocalEccNum
            ...
            /// @}
                try {
                    ...
                    if (Phone.CS_FALLBACK.equals(e.getMessage()) ||
                            (isEmergency && needRetryCsEmergency())) {
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_CRSS_IND:
                ...
                if (noti.code == MtkSuppCrssNotification.CRSS_CALLING_LINE_ID_PREST) {
                    // update numberPresentation in gsmconnection
                    if (getRingingCall().getState() != GsmCdmaCall.State.IDLE) {
                        List<Connection> conns = getCSRingingCall().getConnections();
                        if (conns.isEmpty()) {
                            Rlog.d(LOG_TAG,
                                "Cannot set number presentation to connection (CS conn empty): "
                                        + noti.cli_validity);
                        } else {
                            MtkGsmCdmaConnection cn = (MtkGsmCdmaConnection) (conns.get(0));
                            /* CLI validity value,
                            ...
                            */
                            ...
                            switch (noti.cli_validity) {
                                case 1:
                                    cn.setNumberPresentation(
                                            PhoneConstants.PRESENTATION_RESTRICTED);
                                    break;
                ...
                } else if (noti.code == MtkSuppCrssNotification.CRSS_CONNECTED_LINE_ID_PREST) {
            ...
            case EVENT_SPEECH_CODEC_INFO:
                ...
                ar = (AsyncResult) msg.obj;
                Rlog.d(LOG_TAG, "handle EVENT_SPEECH_CODEC_INFO : " + (int) ar.result);
                notifySpeechCodecInfo((int) ar.result);
                break;
            /// @}
            ...
            case EVENT_GET_CALL_FORWARD_DONE:
                ...
                if (ar.exception == null) {
                    ...
                    if (supportMdAutoSetupIms()) {
                        ...
                        if (cmdException.getCommandError() == CommandException.Error.OEM_ERROR_1) {
                            if (cmdException.getMessage() != null &&
                                ...
                                if (ssHelper != null) {
                                    errorMsg = ssHelper.getXCAPErrorMessageFromSysProp(
                                            CommandException.Error.OEM_ERROR_1);
            ...
            case EVENT_SET_NETWORK_MANUAL_COMPLETE:
                ...
                if (isPhoneTypeGsm()) {
                    ...
                    if (!restoreSelection && exception) {
                        // Swith to auto mode without triggering network selection.
    ...
    public boolean handlePinMmi(String dialString) {
        ...
        if (isPhoneTypeGsm()) {
            ...
            UiccProfile uiccProfile = getUiccProfile();
            if (uiccProfile != null &&
                    uiccProfile.getApplication(UiccController.APP_FAM_3GPP) != null) {
                mmi = CdmaMmiCode.newFromDialString(dialString, this,
                        uiccProfile.getApplication(UiccController.APP_FAM_3GPP));
            } else {
    ...
    public void sendUssdResponse(String ussdMessge) {
        if (isPhoneTypeGsm()) {
            MtkGsmMmiCode mmi = MtkGsmMmiCode.newFromUssdUserInput(
                    ussdMessge, this, mUiccApplication.get());
            mPendingMMIs.add(mmi);
        ...
        /// @}
            ...
            Rlog.d(LOG_TAG, "getCallForwardingOptionForServiceClass enter, " +
                ...
                if ((imsPhone != null)
                    ...
                    ((MtkImsPhone) imsPhone).getCallForwardingOption(commandInterfaceCFReason,
                            onComplete);
                    return;
                }
            ...
            if (supportMdAutoSetupIms()) {
                ...
                if ((isValidCommandInterfaceCFAction(commandInterfaceCFAction)) &&
                    ...
                    if (commandInterfaceCFReason == CF_REASON_UNCONDITIONAL) {
                        // MTK added - start
                        int origUtCfuMode = 0;
                        String utCfuMode = TelephonyManager.getTelephonyProperty(getPhoneId(),
...
//                              isCfEnable(commandInterfaceCFAction) ? 1 : 0, 0, cfu);
    ...
    public CommandException checkUiccApplicationForCB() {
        if (mUiccApplication.get() == null) {
            ...
            if ((!mCi.getRadioState().isAvailable() || !mCi.getRadioState().isOn())) {
            ...
            if ((cmdException != null) && (cmdException.getCommandError()
                ...
                if ((cmdException != null) && (cmdException.getCommandError()
                    ...
                    if (isOpTransferXcap404()
                        ...
                        == CommandException.Error.OEM_ERROR_1)) {
                    //CommandException.Error.OEM_ERROR_1 is used for 409 conflict.
     ...
     */
    public void onMMIDone(MmiCode mmi, Object obj) {
        /* Only notify complete if it's on the pending list.
         * Otherwise, it's already been handled (eg, previously canceled).
         * The exception is cancellation of an incoming USSD-REQUEST, which is
         * not on the list.
         */
        logd("onMMIDone USSD Response:" + mmi + ", obj=" + obj);
        dumpPendingMmi();
        if (mPendingMMIs.remove(mmi) || (isPhoneTypeGsm() && (mmi.isUssdRequest() ||
                ((MtkGsmMmiCode)mmi).isSsInfo()))) {
            ...
            if (receiverCallback != null) {
                ...
                mMmiCompleteRegistrants.notifyRegistrants(new AsyncResult(obj, mmi, null));
            }
        }
    }
    ...
    protected void onIncomingUSSD (int ussdMode, String ussdMessage) {
        ...
        isUssdRequest     = (ussdMode == USSD_MODE_REQUEST);
        isUssdRelease     = (ussdMode == USSD_MODE_NW_RELEASE);
        isUssdhandleByStk = (ussdMode == USSD_HANDLED_BY_STK);
        /* [mtk04070][111118][ALPS00093395]MTK modified */
        isUssdError       = (ussdMode == USSD_OPERATION_NOT_SUPPORTED ||
                             ussdMode == USSD_NETWORK_TIMEOUT);

        /* See comments in GsmMmiCode.java
         * USSD requests aren't finished until one
         * of these two events happen
         */
        MtkGsmMmiCode found = null;
        Rlog.d(LOG_TAG, "USSD:mPendingMMIs= " + mPendingMMIs + " size=" + mPendingMMIs.size());
        for (int i = 0, s = mPendingMMIs.size(); i < s; i++) {
            Rlog.d(LOG_TAG, "i= " + i + " isPending="
                    + ((MtkGsmMmiCode)mPendingMMIs.get(i)).isPendingUSSD());
            if(((MtkGsmMmiCode)mPendingMMIs.get(i)).isPendingUSSD()) {
                found = (MtkGsmMmiCode)mPendingMMIs.get(i);
                Rlog.d(LOG_TAG, "found = " + found);
                break;
            }
        }

        /* If found a pending MMI request */
        if (found != null) {
            ...
            if (isUssdRequest) {
                ...
                /* Fix ALPS03478291, do not display any MMI dialog for +CUSD: 3,
            ...
            if (!isUssdError && ussdMessage != null) {
                ...
                MtkGsmMmiCode mmi;
                mmi = MtkGsmMmiCode.newNetworkInitiatedUssd(ussdMessage,
                                                    isUssdRequest,
                                                    MtkGsmCdmaPhone.this,
                                                    mUiccApplication.get());
                onNetworkInitiatedUssd(mmi);
            }
            /* [mtk04070][111118][ALPS00093395]MTK added */
    ...
    private boolean isUtError(CommandException.Error error) {
        ...
        * CME_NETWORK_TIMEOUT(UT_UNKNOWN_HOST)    CommandException.Error.NO_NETWORK_FOUND
       */
       if (error == CommandException.Error.OEM_ERROR_1 ||
    ...
    public boolean isImsUseEnabled() {
        boolean imsUseEnabled =
                (MtkImsManager.isEnhanced4gLteModeSettingEnabledByUser(mContext, mPhoneId) ||
                MtkImsManager.isWfcEnabledByUser(mContext, mPhoneId) &&
                MtkImsManager.isNonTtyOrTtyOnVolteEnabled(mContext, mPhoneId));
    ...
    public CellLocation getCellLocation(WorkSource workSource) {
        // Add lock to avoid cell location's ClaseCastException and get cell location not match
        // with phone type.
        synchronized (mLock) {
            if (!isPhoneTypeGsm()) {
    ...
    public void getSuppServProperty(String name) {
        Rlog.d(LOG_TAG, "getSuppServProperty, name = " + name);
        mMtkCi.getSuppServProperty(name, null);
    }
    ...
    // M:  ALPS04064723: No need to retry CS ECC when IMS call exist.
    private boolean needRetryCsEmergency() {
        if (mImsPhone != null && mImsPhone instanceof ImsPhone) {
            return !((ImsPhone) mImsPhone).isInCall();
        }
        return true;
    }
