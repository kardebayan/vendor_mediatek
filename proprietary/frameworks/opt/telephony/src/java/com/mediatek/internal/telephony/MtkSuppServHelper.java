...
public class MtkSuppServHelper extends Handler {
    ...
    private static final int EFCFF_STATUS_NOT_READY     = 0;
    private static final int EFCFF_STATUS_VALID         = 2;
    private static final int EFCFF_STATUS_INVALID       = 3;
    ...
    private boolean mNotifyCfuIfEfcfisAndEfcffInvalidDone = false;
    ...
    private void onUpdateIcc() {
        ...
        if (r != newIccRecords) {
            ...
            mNotifyCfuIfEfcfisAndEfcffInvalidDone = false;
    ...
    private void handleSuppServInit() {
        ...
        if (mySubInfo != null && !mySubInfo.getIccId().equals(oldIccId)) {
            ...
            if (isNeedSyncSysPropToSIMforOTA()) {
                ...
                // ALPS03852275: Need to check SIM LOADED.
                int checkEfCfis = checkEfCfis();
                int checkEfCff = checkEfCff();
                logd("Notify CFU ICON, efcfis status: " + checkEfCfis
                        + ", efcff status: " + checkEfCff);

                if (getSimRecordsLoaded()) {
                    if ((checkEfCfis == EFCFIS_STATUS_INVALID && checkEfCff == EFCFF_STATUS_INVALID)
                            && !mNotifyCfuIfEfcfisAndEfcffInvalidDone) {
                        int cfuStatusFromSysProp = getCFUStatusFromLocal();
                        boolean cfuStatus =
                                cfuStatusFromSysProp == IccRecords.CALL_FORWARDING_STATUS_ENABLED ?
                                true : false;
                        logd("Notify CFU icon: " + cfuStatus);
                        mPhone.setVoiceCallForwardingFlag(1, cfuStatus, "");

                        mNotifyCfuIfEfcfisAndEfcffInvalidDone = true;
                    }
                }
            }
        } else if (cfuSetting.equals(CFU_SETTING_ALWAYS_QUERY)) {
    ...
    private void startCFUQuery(boolean bForceNoTimeSlot) {
        ...
        boolean bDataEnable = mPhone.isDataEnabled();
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_SIM_RECORDS_LOADED: {
                ...
                // ALPS03852275: Need to check for SUBINFO Update is ready.
                String mySettingName = CFU_QUERY_ICCID_PROP + mPhone.getPhoneId();
                String oldIccId = SystemProperties.get(mySettingName, "");
                SubscriptionManager subMgr = SubscriptionManager.from(mContext);
                SubscriptionInfo mySubInfo = null;
                ...
                if (mySubInfo != null && mySubInfo.getIccId().equals(oldIccId)) {
                    int checkEfCfis = checkEfCfis();
                    int checkEfCff = checkEfCff();
                    logd("Sim loaded, notify cfu icon, efcfis status: " + checkEfCfis
                            + ", efcff status: " + checkEfCff);
                    if ((checkEfCfis == EFCFIS_STATUS_INVALID && checkEfCff == EFCFF_STATUS_INVALID)
                            && !mNotifyCfuIfEfcfisAndEfcffInvalidDone) {
                        int cfuStatusFromSysProp = getCFUStatusFromLocal();
    ...
    private int checkEfCff() {
        IccRecords r = mPhone.getIccRecords();
        if (r != null &&
                r instanceof MtkSIMRecords) {
            if (((MtkSIMRecords) r).checkEfCff()) {
                return EFCFF_STATUS_VALID;
            } else {
                return EFCFF_STATUS_INVALID;
            }
        }

        // Default is true to available the API is not available.
        return EFCFF_STATUS_NOT_READY;
    }
    ...
    private boolean syncSysPropToSIMforOTA() {
        ...
        if (checkEfCfis == EFCFIS_STATUS_NOT_READY) {
            ...
            int cfuStatusFromSysProp = getCFUStatusFromLocal();
            boolean cfuStatus = false;
            if (cfuStatusFromSysProp == IccRecords.CALL_FORWARDING_STATUS_ENABLED) {
                cfuStatus = true;
                logd("syncSysPropToSIMforOTA: true from system property.");
                mPhone.setVoiceCallForwardingFlag(1, cfuStatus, "");
            } else if (cfuStatusFromSysProp == IccRecords.CALL_FORWARDING_STATUS_DISABLED) {
    ...
    private ImsConnectionStateListener mImsConnectionStateListener =
        ...
        public void onImsConnected(int imsRadioTech) {   // STATE_IN_SERVICE
            ...
            if (cfuSetting.equals(CFU_SETTING_ALWAYS_NOT_QUERY)) {
                logd("onImsConnected, no need to query CFU over IMS due to ALWAYS_NOT_QUERY");
                return;
            }

            if (!isNotMachineTest()) {
                logd("onImsConnected, no need to query CFU over IMS due to machine test");
                return;
            }
    ...
    private int getCFUStatusFromLocal() {
        String cfuMode = TelephonyManager.getTelephonyProperty(mPhone.getPhoneId(),
                PROPERTY_UT_CFU_NOTIFICATION_MODE, UT_CFU_NOTIFICATION_MODE_DISABLED);

        logd("getCFUStatusFromLocal: " + cfuMode);

        if (UT_CFU_NOTIFICATION_MODE_ON.equals(cfuMode)) {
            return IccRecords.CALL_FORWARDING_STATUS_ENABLED;
        } else if (UT_CFU_NOTIFICATION_MODE_OFF.equals(cfuMode)) {
            return IccRecords.CALL_FORWARDING_STATUS_DISABLED;
        }

        return IccRecords.CALL_FORWARDING_STATUS_UNKNOWN;
    }
