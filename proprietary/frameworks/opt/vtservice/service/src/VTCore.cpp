...
namespace VTService {
    ...
    status_t VTCore::updateTxRxMode(const int id, int new_mode, vt_srv_call_update_info_struct *info) {
        ...
        } while (loop_counter < 5 && (new_mode & (*setSfState)) != new_mode); // all surface need in this mode is set
    ...
    status_t VTCore::snapshot(int id, VT_SRV_SNAPSHOT_TYPE type, String8 savingImgURI) {
        mHelper->logAction("snapshot", "id", id, mHelper->getSnapshotString(type), VT_SRV_LOG_I);

        VT_SRV_MA_STATE state = mHelper->getState(id);
        mLastRet = mHelper->checkRange(state, VT_SRV_MA_STATE_OPENED, VT_SRV_MA_STATE_START_UD);
        if (mLastRet) {
            mHelper->logAction("snapshot", "id", id, "call has stopped", VT_SRV_LOG_E);
            goto error;
        }
        ...
        mLastRet = mHelper->get(id)->SnapShot(savingImgURI.string(), (snapshot_mode_t) type);
        if (mLastRet) goto error;

        return VT_SRV_RET_OK;

        error:
            notifyError(id, "snapshot");
            return mLastRet;
    }

    status_t VTCore::startRecording(int id, VT_SRV_RECORD_TYPE type, String8 path, int maxSize) {
        mHelper->logAction("startRecording", "id", id, mHelper->getRecordString(type), VT_SRV_LOG_I);

        VT_SRV_MA_STATE state = mHelper->getState(id);
        mLastRet = mHelper->checkRange(state, VT_SRV_MA_STATE_OPENED, VT_SRV_MA_STATE_START_UD);
        if (mLastRet) {
            mHelper->logAction("startRecording", "id", id, "call has stopped", VT_SRV_LOG_E);
            goto error;
        }
        ...
        mLastRet = mHelper->get(id)->SetRecordParameters(MA_RECQUALITY_MEDIUM, (char *)path.string());
        if (mLastRet) goto error;

        mLastRet = mHelper->get(id)->StartRecord((record_mode_t) type);
        if (mLastRet) goto error;

        return VT_SRV_RET_OK;

        error:
            notifyError(id, "startRecording");
            return mLastRet;
    }

    status_t VTCore::stopRecording(int id) {
        mHelper->logAction("stopRecording", "id", id, "", VT_SRV_LOG_I);

        VT_SRV_MA_STATE state = mHelper->getState(id);
        mLastRet = mHelper->checkRange(state, VT_SRV_MA_STATE_OPENED, VT_SRV_MA_STATE_START_UD);
        if (mLastRet) {
            mHelper->logAction("stopRecording", "id", id, "call has stopped", VT_SRV_LOG_E);
            goto error;
        }

        mLastRet = mHelper->get(id)->StopRecord();
        if (mLastRet) goto error;

        return VT_SRV_RET_OK;

        error:
            notifyError(id, "stopRecording");
            return mLastRet;
    }
