...
public class MtkBearerData extends BearerData {
    ...
    public static byte[] encode(BearerData bData) {
        ...
        UserData temp = null;
        /* Support 7bit ASCII and Octet EMS encoding */
        if (isMtkEmsEncodingExt(bData)) {
            userData = encodeUserData(bData);
            if (userData == null) {
                return null;
            }
            temp = bData.userData;
            /* Clear the userData, because we encode the user data by us
               and we let the BearerData.encode encode other parameter and
               not encode the userData */
            bData.userData = null;
        }
        ...
        if (bData.hasUserDataHeader) {
            final int headerMask = 0x08;
            final int headerIndex = 4;
            main[headerIndex] = (byte) (((main[headerIndex] & 0xFF) | headerMask) & 0xFF);
        }
        /* Support to save time in SIM */
        byte[] append = encodeMsgCenterTimeStamp(bData);
        byte[] result = new byte[main.length + userData.length + append.length];
        System.arraycopy(main, 0, result, 0, main.length);
        System.arraycopy(userData, 0, result, main.length, userData.length);
        System.arraycopy(append, 0, result, main.length + userData.length, append.length);
        return result;
    }
    ...
    private static boolean isMtkEmsEncodingExt(BearerData bData) {
        if (bData.userData == null ||
                bData.userData.userDataHeader == null ||
                !bData.userData.msgEncodingSet ||
                ((bData.userData.msgEncoding != UserData.ENCODING_7BIT_ASCII) &&
                 (bData.userData.msgEncoding != UserData.ENCODING_OCTET))) {
            return false;
        }
        return true;
    }

    private static byte[] encodeUserData(BearerData bData) {

        if ((bData.userData.payloadStr == null) &&
                (bData.userData.msgEncoding != UserData.ENCODING_OCTET)) {
        ...
        try {
            ...
            byte[] headerData = SmsHeader.toByteArray(bData.userData.userDataHeader);
            if (bData.userData.msgEncoding == UserData.ENCODING_7BIT_ASCII) {
                encode7bitAsciiEms(bData.userData, headerData, true);
            }
            ...
            Rlog.e(LOG_TAG, "UserData encode failed: " + ex);
    ...
    /**
     * Support encode Ems with the 7bitAscii encoding type.
     * @param uData The user data.
     * @param udhData The user data header.
     * @param force If force to use 7bit encode.
    ...
    private static void encode7bitAsciiEms(UserData uData, byte[] udhData, boolean force)
        ...
        try {
            BitwiseOutputStream outStream =
                    new BitwiseOutputStream((udhSeptets + msgSeptets) * 7);
            ...
            for (int i = 0; i < msgSeptets; i++) {
                ...
                if (charCode == -1) {
                    if (force) {
                        ...
                        throw new CodingException("cannot ASCII encode ("
                                                  + uData.payloadStr.charAt(i) + ")");
                    }
                } else {
