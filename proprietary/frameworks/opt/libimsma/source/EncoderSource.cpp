...
EncoderSource::Rotator::Rotator(int32_t multiId,const sp<ALooper> &looper)
    : mMultiInstanceID(multiId),
      ...
      mStop(false)
{
    VT_LOGI("[ID=%d][Rotator]create+++",mMultiInstanceID);
    ...
    case kWhatSetParameters: {
        ...
        if(msg->findMessage("codec", &params)) {
            ...
            if(params->findInt32("i-frame-interval",   &value1)) {
                mOutputFormat->setInt32("i-frame-interval", value1);
            }
...
status_t EncoderSource::feedEncoderInputBuffersAfterRotate()
{
    ...
    while(!mInputABufferQueue.empty()) {
        ...
        VT_LOGV("[ID=%d][EncS][profile:token %d]queue Encoder InputBuffer :timeUs %lld us longtime flag:%d",
