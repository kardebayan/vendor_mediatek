...
final public class MtkIpConnectivityMetrics {
    ...
    private final INetdEventCallback mNetdEventListener = new BaseNetdEventCallback() {
        ...
        public void onDnsEvent(String hostname, String[] ipAddresses, int ipAddressesCount,
            ...
            mImpl.onCtaDnsEvent(uid);
            mImpl.onMonitorDnsEvent(hostname, ipAddressesCount, uid);
        }
    ...
    public final class Impl extends IMtkIpConnectivityMetrics.Stub {
        ...
        private void onCtaDnsEvent(int uid) {
            ...
            synchronized (mUidSockeRulestLock) {
                ...
                if (isNotified) {
                    NetworkCapabilities nc = mCm.getNetworkCapabilities(mCm.getActiveNetwork());
                    if (nc == null) {
                        return;
                    }
        ...
        private void onMonitorDnsEvent(String hostname, int ipAddressesCount, int uid) {
            if (mSocketEventCallback == null) return;
            try {
                mSocketEventCallback.onDnsEvent(hostname, null, ipAddressesCount, 0L, uid);
            } catch (Exception e) { }
        }
