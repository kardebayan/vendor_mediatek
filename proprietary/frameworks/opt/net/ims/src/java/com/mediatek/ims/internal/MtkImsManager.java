...
public class MtkImsManager extends ImsManager {
    ...
    private ArrayList<ImsRegistrationImplBase.Callback> mCallbacks
            = new ArrayList<ImsRegistrationImplBase.Callback>();
     ...
     */
    public static void setVoltePreferSetting(Context context, int mode,
            int phoneId) {
        if (isSupportMims() == false) {
            phoneId = getMainPhoneIdForSingleIms(context);
        }
        getAppropriateManagerForPlugin(context, phoneId)
                .setVoltePreferSetting(mode);
    }
    ...
    protected boolean isDataRoaming() {
        if (SystemProperties.getInt(MULTI_IMS_SUPPORT, 1) == 1) {
            ...
            return SystemProperties.getBoolean(DATA_ROAMING_PROP + String.valueOf(phoneId), false);
        }
        return SystemProperties.getBoolean(DATA_ROAMING_PROP + String.valueOf(mPhoneId), false);
     ...
     */
    public void setDataRoaming(boolean enabled) {
        if (SystemProperties.getInt(MULTI_IMS_SUPPORT, 1) == 1) {
            int phoneId = getMainCapabilityPhoneId(mContext);
            log("[" + phoneId + "] setDataRoamingEnabled: " + enabled);

            SystemProperties.set(DATA_ROAMING_PROP + String.valueOf(phoneId), enabled ? TRUE : FALSE);
            return;
        }

        log("[" + mPhoneId + "] setDataRoamingEnabled: " + enabled);
        SystemProperties.set(DATA_ROAMING_PROP + String.valueOf(mPhoneId), enabled ? TRUE : FALSE);
    }
    ...
    protected boolean isDataRoamingSettingsEnabled() {
        if (SystemProperties.getInt(MULTI_IMS_SUPPORT, 1) == 1) {
            int phoneId = getMainCapabilityPhoneId(mContext);
            return SystemProperties.getBoolean(DATA_ROAMING_SETTING_PROP + String.valueOf(phoneId), false);
        }
        return SystemProperties.getBoolean(DATA_ROAMING_SETTING_PROP + String.valueOf(mPhoneId), false);
    }
    ...
    public void setDataRoamingSettingsEnabled(boolean enabled) {
        if (SystemProperties.getInt(MULTI_IMS_SUPPORT, 1) == 1) {
            int phoneId = getMainCapabilityPhoneId(mContext);
            log("[" + phoneId + "] setDataRoamingSettingsEnabled: " + enabled);

            SystemProperties.set(DATA_ROAMING_SETTING_PROP + String.valueOf(phoneId), enabled ? TRUE : FALSE);
            return;
        }

        log("[" + mPhoneId + "] setDataRoamingSettingsEnabled: " + enabled);
        SystemProperties.set(DATA_ROAMING_SETTING_PROP + String.valueOf(mPhoneId), enabled ? TRUE : FALSE);
    }
    ...
    public static void setWfcSetting(Context context, boolean enabled,
        ...
        getAppropriateManagerForPlugin(context, phoneId)
                .setWfcSetting(enabled);
    }

    public void setWfcSetting(boolean enabled) {
        log("setWfcSetting() : " + enabled);
        ...
        boolean isNetworkRoaming = tm.isNetworkRoaming(getSubId());
        if (isNetworkRoaming && isConvertRoamingStateForSpecificOP()) {
            isNetworkRoaming = false;
        }
        setWfcNonPersistent(enabled, getWfcMode(isNetworkRoaming));
    }
    ...
    public void setWfcNonPersistent(boolean enabled, int wfcMode) {
        ...
        try {
            ...
            try {
                ...
                Log.e(TAG, "setWfcNonPersistent: isCameraAvailable() Exception: " + e.getMessage());
            }
            ...
            boolean enableViWifi = (!isViWifiNeedCheckWfcEnabled || enabled)
            ...
            changeMmTelCapability(MmTelFeature.MmTelCapabilities.CAPABILITY_TYPE_VIDEO,
                    ImsRegistrationImplBase.REGISTRATION_TECH_IWLAN, enableViWifi);
    ...
    public static void setWfcMode(Context context, int wfcMode, int phoneId) {
        if (DBG)
            log("setWfcMode(), setting=" + wfcMode + ", phoneId:" + phoneId);
        if (isSupportMims() == false) {
            phoneId = getMainPhoneIdForSingleIms(context);
        }
        getAppropriateManagerForPlugin(context, phoneId)
    ...
    public static void setWfcMode(Context context, int wfcMode,
            boolean roaming, int phoneId) {
        if (DBG)
            log("setWfcMode(), wfcMode: " + wfcMode + ", roaming:" + roaming
                    + ", phoneId:" + phoneId);
        if (isSupportMims() == false) {
            phoneId = getMainPhoneIdForSingleIms(context);
        }
        getAppropriateManagerForPlugin(context, phoneId)
                .setWfcMode(wfcMode, roaming);
    }
    ...
    public static void setWfcRoamingSetting(Context context, boolean enabled,
            int phoneId) {
        if (DBG)
            log("setWfcRoamingSetting(), enabled: " + enabled + ", phoneId:"
                    + phoneId);
   ...
   */
    ...
    protected boolean updateVideoCallFeatureValue() throws ImsException {
        ...
        try {
            ...
            throw new ImsException("isCameraAvailable()", e,
                    ImsReasonInfo.CODE_LOCAL_IMS_SERVICE_DOWN);
        }
        ...
        changeMmTelCapability(
                ...
                ImsRegistrationImplBase.REGISTRATION_TECH_LTE, isVilteFeatureOn);
        ...
        boolean isViWifiNeedCheckVolteEnabled = getBooleanCarrierConfig(
                MtkCarrierConfigManager.MTK_KEY_VT_OVER_WIFI_CHECK_VOLTE_ENABLE_BOOL);
        boolean viwifiEnabled = (!isViWifiNeedCheckVolteEnabled || isEnhanced4gLteModeSettingEnabledByUser())
                && isVtEnabledByUser()
        ...
        changeMmTelCapability(
                MmTelFeature.MmTelCapabilities.CAPABILITY_TYPE_VIDEO,
                ImsRegistrationImplBase.REGISTRATION_TECH_IWLAN, isViwifiFeatureOn);

        return (isVilteFeatureOn || isViwifiFeatureOn);
    }
    ...
    protected boolean isImsResourceSupport(int feature) {
        ...
        if ("1".equals(SystemProperties.get(PROPERTY_DYNAMIC_IMS_SWITCH))) {
            ...
            if (!SubscriptionManager.isValidPhoneId(mPhoneId)) {
                loge("isImsResourceSupport() Invalid main phone " + mPhoneId
    ...
    /**
     * MIMS Resets ImsManager settings back to factory defaults.
     * @hide
     */
    public static void factoryReset(Context context, int phoneId) {
        ...
        if (isSupportMims() == false) {
            ...
            /// M: CT VOLTE setting is managed by sim, no need reset if it is
            // not main sim @{
            ...
            /// @}
            ((MtkImsManager) ImsManager.getInstance(context, mainPhoneId))
                    .factoryReset();
        } else {
            /*
             * Reset specific phone IMS setting for MIMS project VoLTE Setting:
             * ImsConfig.FeatureValueConstants.ON WFC Setting:
             * CarrierConfigManager.KEY_CARRIER_DEFAULT_WFC_IMS_ENABLED_BOOL WFC
             * MODE Setting:
             * CarrierConfigManager.KEY_CARRIER_DEFAULT_WFC_IMS_MODE_INT) WFC
             * Roaming Setting:
    ...
    public ImsCall takeCall(IImsCallSession session, Bundle incomingCallExtras,
        ...
        try {
            ...
            IMtkImsCallSession mtkSession = mMtkImsService.getPendingMtkCallSession(callId);
    ...
    protected void setLteFeatureValues(boolean turnOn) {
        ...
        if (isVolteEnabledByPlatform() && isVtEnabledByPlatform()) {
            ...
            boolean isViWifiNeedCheckVolteEnabled = getBooleanCarrierConfig(
                    MtkCarrierConfigManager.MTK_KEY_VT_OVER_WIFI_CHECK_VOLTE_ENABLE_BOOL);
            boolean viwifiEnabled = (turnOn || !isViWifiNeedCheckVolteEnabled)
                    && isVtEnabledByUser()
                    && (!isViWifiNeedCheckWfcEnabled || isWfcEnabledByUser());
            boolean isViWifiNeedCheckDataEnabled = getBooleanCarrierConfig(
                    MtkCarrierConfigManager.MTK_KEY_VT_OVER_WIFI_CHECK_DATA_ENABLE_BOOL);
    ...
    private boolean isFeatureEnabledByPlatformExt(int feature) {
        ...
        isEnabled = sImsManagerExt.isFeatureEnabledByPlatform(mContext,
                feature, mPhoneId);
        return isEnabled;
    }
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (CarrierExpressManager.ACTION_OPERATOR_CONFIG_CHANGED
                    .equals(intent.getAction())) {
                log("[onReceive] intent =" + intent.getAction());
                // Need to get op plug in again after carrier express switch done
                sImsManagerExt = null;
                getImsManagerPluginInstance(context);;
            }
            /// @}
    ...
    private static MtkImsManager getAppropriateManagerForPlugin(Context context,
            int phoneId) {
        if (sImsManagerExt == null) {
            MtkImsManager.getImsManagerPluginInstance(context);
        }
        if (sImsManagerExt != null) {
        ...
        switch (event) {
            case SERVICE_REG_CAPABILITY_EVENT_ECC_SUPPORT:
                synchronized (mCallbacks) {
                    for (ImsRegistrationImplBase.Callback callback : mCallbacks) {
    ...
    private boolean isLteSupported() {
        String ps1Rat = SystemProperties.get("ro.boot.opt_ps1_rat", "");
    ...
    public void setEnhanced4gLteModeVtSetting(
        ...
        if (imsManager != null) {
            try{
                ...
                if (isSupportMims() == false) {
                    prevSetting = isEnhanced4gLteModeSettingEnabledByUser(context, mPhoneId)? 1 : 0;
                    SubscriptionManager.setSubscriptionProperty(getSubId(),
                                SubscriptionManager.ENHANCED_4G_MODE_ENABLED,
                                Integer.toString(volte_value));
                    SubscriptionManager.setSubscriptionProperty(getSubId(),
                                SubscriptionManager.VT_IMS_ENABLED,
                                Integer.toString(vt_value));
                } else {
                ...
                if (e4genabled || vtenabled) {
                    ...
                    turnOnIms();
                                ...
                                && (!imsManager.isVolteEnabledByPlatform()
                                    || !isEnhanced4gLteModeSettingEnabledByUser(context, mPhoneId))
    ...
    public void removeCapabilityCallback(ImsFeature.CapabilityCallback callback)
        ...
        try {
            ...
            throw new ImsException("removeCapabilityCallback(IF)", e,
                    ImsReasonInfo.CODE_LOCAL_IMS_SERVICE_DOWN);
        }
    }
    ...
    private void hookProprietaryImsListener() throws ImsException {
        ...
        try {
            if (mMtkImsService != null) {
                mMtkImsService.registerProprietaryImsListener(mPhoneId, mListener);
            } else {
                log("mMtkImsService is not ready yet");
            }
        } catch (RemoteException e) {
    ...
    public void addImsConnectionStateListener(
            ImsRegistrationImplBase.Callback callback) throws ImsException {
